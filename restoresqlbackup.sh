...
list_backups() {
    ...
    # Robust Validation: Check for any 3-letter month and 4-digit year
...
restore_backup() {
    echo "************************************************"
    echo "Restoring database backup for $app_name..."
    echo "Source Time: $formatted_date"
    echo "************************************************"

    # Restoring with -d flag for database
    /var/cw/scripts/bash/duplicity_restore.sh --src "$app_name" -v 4 -d --dst './' --time "$formatted_date"
    
    if [ $? -eq 0 ]; then
        echo "SUCCESS: Database backup restoration completed."
    else
        echo "FAILURE: Database restoration encountered an error."
    fi
}
...
fix_permissions() {
    if [ -d "mysql" ]; then
        ...
        echo "Permissions updated for $app_name:www-data"
        echo "************************************************"
    fi
}
